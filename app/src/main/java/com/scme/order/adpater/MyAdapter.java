...
public class MyAdapter extends RecyclerView.Adapter<MyAdapter.MViewHolder> {
	...
	private List<Photoimage> listData;
	private ItemClickListener mItemClickListener;

	public MyAdapter(Context context, List<Photoimage> mList) {
		super();
	...
	public void addData(Photoimage info, int position) {
	...
	public void addData(Photoimage info) {
		// listData.add(position, info);
		// notifyItemInserted(position);
		listData.add(info);
	 ...
	 */
	public void itemRangeInserted(Photoimage info, int positionStart, int itemCount) {
		for (int i = positionStart; i < itemCount; i++) {
			listData.add(i, info);
		}
	...
	public void onBindViewHolder(final MViewHolder mViewHolder,
		...
		mViewHolder.mTextView.setText(listData.get(postion).getPath());
